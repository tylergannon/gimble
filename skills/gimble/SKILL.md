---
name: gimble
description: Run coding agents as detached pipelines with Gimble — loop until a check actually passes, fan out across Claude/Codex/Gemini, or have a second model review the work. Use when the user says "don't stop until the tests pass", "loop on this until it works", "keep working while I'm gone", "keep going after I close my laptop", "run this in the background", "try a couple of approaches in parallel", "have another model check this", "get a second opinion from Codex or Gemini" — or asks to start, check on, steer, or stop a Gimble run. Starts from copy-and-run examples; no graph authoring needed. Do not use for ordinary single-agent work that finishes in this session.
---

# Gimble

Gimble runs pipelines of real coding agents — the Claude Code, Codex, and
...
They ship beside this file, mirroring `examples/loops/` in the Gimble repo;
...
the binary and run by name, with nothing to copy. `gimble workflows` lists
...
the gate, so they work in any language. `gimble workflows show <name>` prints the
...
4. Report the returned `run_id`, process ID, and log paths. Run state lives
   under `~/.local/state/gimble/mcp-runs` and outlives this session and any
...
to this exact parent so Gimble can queue bounded news directly back to it.
...
workflow must reach. Gimble runs the whole Procfile through Overmind for the
run. Overmind supplies each process with `PORT`; Gimble exports the named
process's port as `GIMBLE_SERVICE_PORT` to every workflow shell. Construct the
...
```sh
GIMBLE_INTERVIEW_DIR=ephemeral/projects/<build>/interview gimble ask question.md
```

The command moves the question to a numbered file, records `QuestionAsked`
when `GIMBLE_RUN_DIR` is available, and prints the answer once another
process runs `gimble answer <numbered-question> [text]`. Answers may come
from stdin; an existing answer is never overwritten. If a shell session ends
while waiting, rerun `gimble ask` with the numbered question path to resume.

On the calling side, watch the returned run path's `timeline.jsonl` for
`QuestionAsked`, open the event's `question` path, then run `gimble answer`
...
provider and harness are derived. `gimble inspect-models <file>` displays the
effective selection and provenance for every node and hidden loop role. Keep
each prompt to the decision its node owns.

`gimble edit <file>` opens the pipeline in a browser graph editor with the
