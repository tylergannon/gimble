...
watch over a Tractor run.

Read `CODEX_THREAD_ID` and `CODEX_SESSION_ID` from this task's shell. Those
values belong to the caller; environment inherited by the shared Tractor MCP
...
That explicit attachment opts the run into direct Codex delivery. Tractor's
...
do not claim enqueue alone reopened a cold task. Tractor records rejected queue
