...
`--workdir`; planning artifacts land under that workdir. The logs directory is
where you watch the live run.

## Answer the interview

Watch `./tractor-plan-logs/timeline.jsonl`. When a `QuestionAsked` event
...
MEDIUM or LARGE plan, run its exact `Next` command only when that workflow is
shown by `tractor workflow list`; until then it is a pending handoff. The
[Loops guide](/docs/loops/) explains the execution shapes and checklist
validation without duplicating their graph details here.
