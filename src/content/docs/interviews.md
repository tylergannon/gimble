...
one Markdown or HTML question, runs `tractor ask`, and stays inside the same
...
`tractor ask` moves the question to the next numbered `.md` or `.html` file,
records `QuestionAsked` in the run's `timeline.jsonl`, and blocks. Tractor sets
`TRACTOR_RUN_DIR` for agents and tool commands inside the run, so `ask` can
...
`tractor answer` writes `0001.answer.md` beside the question. Omit the text to
...
```sh
TRACTOR_INTERVIEW_DIR=ephemeral/projects/my-build/interview \
  tractor ask ephemeral/projects/my-build/interview/0001.md
```
