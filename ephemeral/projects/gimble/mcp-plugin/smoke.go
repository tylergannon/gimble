...
func run() error {
	...
	serverConfig, ok := config.Servers["gimble"]
	if !ok {
		return errors.New(".mcp.json has no gimble server")
	}
	...
	initialize.Params.ClientInfo = mcp.Implementation{Name: "gimble-live-proof", Version: "1.0.0"}
	...
	workdir := filepath.Join(repository, "ephemeral", "projects", "gimble", "mcp-plugin", "live-workspace")
	...
	logsRoot, err := os.MkdirTemp("", "gimble-mcp-live-proof-")
	if err != nil {
		return err
	}
