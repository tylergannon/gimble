...
- Open: what survives the Sep 3 cut beyond the loop node, the checklist
  package, frames, the lint rules, and tractor ask and answer. Two of Tyler's
  earlier decisions were never withdrawn: authoring is itself a Tractor run
...
- The Sep 2 decisions file, deleted in 3f83f2c, is readable with git show at
  f7801ba (as first written), 480b6dd (with overnight amendments), and
  cbc0686 (the reset). Its numbering changed between the last two, so cite a
  decision by content, not by number. Decisions 1 to 25 came from the Sep 1
  to 2 interview with Tyler; 26 to 36 were answered by Claude alone; 37
  onward are Claude's write-up of the Sep 2 evening conversation, with
  overnight amendments made by no human.
- Doctrine pages: git show 480b6dd, under
  ephemeral/projects/tractor/living-instructions/chapters/04-library/content/doctrine/.
- Tie-off: ephemeral/projects/tractor/goal-gates-tie-off/notes.md; issues #32
  to #37 and #39; PR #38.
- diffusioninc skills: github.com/diffusioninc/skills (df-chapter-create,
  df-sprint-plan, df-sprint-execute, df-promise).
- Loop-frames memo, section 9a: ephemeral/projects/tractor/loop-frames/memo.md
  (untracked in the main checkout as of Sep 3).
