...
The tool node is the only thing that decides: the lint refuses any agent
edge to `success` (`goal_gate`), so a run can only end on a command's exit
code. Point that command at the closest observable proof of the user's
claim — run the app, curl the endpoint, assert on the artifact. Tests and
linters are worth requiring, but prove the claim only when they exercise
that behavior.
