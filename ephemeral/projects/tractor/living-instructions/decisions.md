...
8. SIMPLE is one sprint. MEDIUM is a flat checklist of sprints. LARGE is
   only for twelve or more sprints, grouped into chapters (diffusioninc's
   rule: a chapter guides 12 to 100 sprints; never make one for less).
   MEDIUM and LARGE begin in the same interview; its exit routes the size.
   (Amended 2026-09-03: the original wording sized in chapters without
   saying how big a chapter is, and v2 got planned as three chapters of
   4 to 14 sprints. See `postmortem-review-loops.md`.)
...
37. **Review ceiling.** A reviewer of a plan or design asks one question:
    could a frontier coding agent, given this plan and nothing else, arrive
    at a correct result? Findings are blocking or note; only blocking
    routes fail. The author fixes, rejects with a reason, or concedes.
    Two laps, then a question to the human. A fix that grows a document by
    more than a fifth is a redesign and stops the loop. Reviewer prompts
    never ask for attacks or make any finding a fail. Reason in
    `postmortem-review-loops.md`.
...
3. Built-in planning workflow, embedded, ends with plan and recommendation.
   **Done** (chapter 2). v2 (interview, plan, review; prompts as files) is
   `planner-v2/sprints.md`, three sprints, MEDIUM.
