...
| P7 | `scope_cop` delivers at least one steer during a planning run, and the steered turn's output differs from what it was writing before the steer. | That supervisors improve plans. | Scenario | Timeline `steer` verdict with a delivered disposition; diff of the stage output before and after; an `infer` judge over the steer text and the target's segment split at the steer, deciding that the change is what the steer asked (decision 41). |
...
| Holdout handoff | design lap → `verify` prompt materialization | The sample lives under `$XDG_STATE_HOME/tractor/holdouts/<build>-<token>/`; its path is recorded outside the workdir at `$XDG_STATE_HOME/tractor/holdouts/<build>.path` by the design lap and read by the execution run when it materializes `verify`; rendered into no other prompt (obscure, not secret; decision 43). |
...
re-planned each lap in runs started from a build that has `replan`
(chapter 6, sprint 1; in the run that builds it, as in chapter 5's, the
