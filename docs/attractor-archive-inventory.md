...
`tylergannon/attractor` repository that Gimble has not adopted. Inclusion here
does not imply that Gimble should implement an idea or preserve its original
shape.
...
and Gimble at `37df42b07c8f3db41b176d51c243646be79eaa1f`. Attractor's
unmerged branches remain available in the archive but are outside this
inventory's implementation claims.

## What Gimble actually loses

Not every missing mechanism makes Gimble worse. The north star deliberately
...
| Archived idea | Cost to Gimble today | Assessment |
| --- | --- | --- |
| [Stable model aliases](https://github.com/tylergannon/gimble/issues/11) | Pipeline authors must pin provider-native names everywhere, so coordinated upgrades are noisy and defaults can drift across entry points. | Clear small gap; preserve raw model IDs as an escape hatch. |
| [Sortable run IDs](https://github.com/tylergannon/gimble/issues/12) | Manifest IDs, directory names, CLI output, and MCP state use separate identity or ordering conventions, making runs harder to correlate by eye and by tooling. | Clear small gap; one opaque sortable ID should travel end to end. |
| [Usage and quota telemetry](https://github.com/tylergannon/gimble/issues/13) | Gimble records native events but cannot answer basic typed questions about token burn, context pressure, or quota reset windows. That blocks budgets and the north star's reserved usage-aware routing work. | Clear gap; normalize the stable common fields while retaining raw provider observations. |
| [Fully generated Codex protocol](https://github.com/tylergannon/gimble/issues/10) | Handwritten JSON-RPC envelopes and response shapes can silently drift from Codex app-server, leave capabilities undiscoverable, and turn protocol changes into runtime failures. | Clear correctness and maintenance gap; the complete checked-in protocol surface should be generated. |
...
| Rich outcome-directed routing and goal gates | None relative to the chosen model: adding them would let the engine interpret agent verdicts and recreate routing policy outside node occupants. | Deliberate exclusion; Gimble is better off without it. |
...
| Native tool-call hooks | None at the Gimble layer: harnesses already own native tool policy, and another interception surface would duplicate it. | Deliberate exclusion. |
| Alternative parallel join policies | Gimble cannot race branches and accept the first success, but doing so creates cancellation, evidence, and workspace-ownership semantics absent from the normative convergence model. | A possible future mode, not a present deficit. |

The first four are the obvious near-term candidates. The middle group names
specific ways Gimble is worse off but still has to clear the cost razor. The
...
Gimble's harness outcome is deliberately smaller (`next` and `notes`), and its
...
Gimble has a compact file-level `defaults` object for six common LLM fields,
...
Gimble can build review-and-repair loops with ordinary edges, but it has no
...
Gimble's command node is intentionally a smaller shell-command primitive. The
...
Gimble preserves native harness events in JSONL but does not expose a typed
...
for the complete protocol surface. Gimble generates and retains only the
request model it currently needs, then uses small hand-written JSON-RPC types
for the rest.

The original Gimble design explicitly chose narrow request generation and a
handwritten observed response surface. That choice leaves Gimble worse off:
...
has a recursive linter for its required strict-output subset. Gimble validates
...
time recoverable and lexical order chronological. Gimble's manifest ID is a
...
IDs while still allowing raw model IDs. Gimble accepts provider and model
...
good pattern if Gimble adopts them. Without one, every authored graph becomes
...
environment setup file that installs Lefthook automatically. Gimble has a
...
execute either node. Gimble has neither concept as a first-class node.
...
and `allow_partial`. The runner explicitly rejects those settings. Gimble has
...
never implemented. Gimble also has no native tool interception layer.
...
Gimble supports adapter interruption but has no equally rich public close
contract.
...
sequential runner does not execute parallel nodes. Gimble implements real
...
carefully as Gimble's current parallel path.

## Things that are already in Gimble
...
They are excluded from the inventory above because Gimble already has them,
