...
func TestRuntimeServesWebApplicationOverUDS(t *testing.T) {
...
func TestRuntimeServesWebApplicationOnSelectedPort(t *testing.T) {
	ctx, cancel := context.WithCancel(t.Context())
	runtime, err := NewRuntime(ctx, t.TempDir(), WithPort(0))
	if err != nil {
		t.Fatal(err)
	}
	response, err := http.Get("http://" + runtime.web + "/")
	if err != nil {
		t.Fatal(err)
	}
