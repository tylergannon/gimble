...
name: release-gimble
description: >
  Release, publish, tag, or bump Gimble and refresh its Codex plugin
  marketplace installation. Use for every Gimble version release, plugin
...
Ship a versioned Gimble release and prove the configured Codex marketplace
...
3. Set `.codex-plugin/plugin.json` `version` to the plain release version on
   ...
   plugin development.
4. Keep `.agents/plugins/marketplace.json` unchanged when its Gimble source
...
python3 "${CODEX_HOME:-$HOME/.codex}/skills/.system/skill-creator/scripts/quick_validate.py" .agents/skills/release-gimble
git diff --check
...
built release binary, append `-args -gimble-binary <absolute-path>` to that
...
4. Verify `codex plugin list` reports `gimble@gimble` installed and enabled
   at the exact manifest version.
5. Start a fresh Codex process and exercise the installed marketplace copy. At
   minimum, call Gimble's deferred `get_pipeline_schema` MCP tool, or run a
