...
func newAskCommand() *cobra.Command {
	...
	command := &cobra.Command{
		...
		RunE: func(command *cobra.Command, args []string) error {
			...
			if !resumed {
				...
				if runDir := os.Getenv("GIMBLE_RUN_DIR"); runDir != "" {
					if err := appendQuestionAsked(runDir, displayPath); err != nil {
						return err
					}
				} else if _, err := fmt.Fprintln(command.ErrOrStderr(), "warning: GIMBLE_RUN_DIR is unset; question was not recorded in a run timeline"); err != nil {
	...
	command.Flags().StringVar(&interviewDir, "into", "", "interview directory (default GIMBLE_INTERVIEW_DIR)")
	return command
}
...
func resolveInterviewDir(flagValue string, flagSet bool) (string, error) {
	...
	if dir := os.Getenv("GIMBLE_INTERVIEW_DIR"); dir != "" {
		return dir, nil
	}
	return "", fmt.Errorf("interview directory is required: use --into or set GIMBLE_INTERVIEW_DIR")
}
