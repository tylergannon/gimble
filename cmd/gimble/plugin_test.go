...
func TestPluginInstallerUpgradesAndReinstallsBeforeCleaningCache(t *testing.T) {
	stateRoot := t.TempDir()
	...
	if err := os.WriteFile(filepath.Join(cache, "old-wrapper"), []byte("old"), 0o755); err != nil {
		t.Fatal(err)
	}
	...
	run := func(name string, arguments ...string) ([]byte, error) {
		...
		case "codex plugin marketplace list --json":
			return []byte(`{"marketplaces":[{"name":"gimble"}]}`), nil
		case "codex plugin marketplace upgrade gimble --json":
			return []byte(`{}`), nil
		case "codex plugin list --json":
			return []byte(`{"installed":[{"pluginId":"gimble@gimble"}]}`), nil
		case "codex plugin remove gimble@gimble --json", "codex plugin add gimble@gimble --json":
			return []byte(`{}`), nil
		default:
	...
	if _, err := os.Stat(cache); !os.IsNotExist(err) {
		t.Fatalf("cache %q still exists: %v", cache, err)
	}
...
func TestRetireUnregisteredMCPProcessesKillsOnlyIdleUnregisteredServers(t *testing.T) {
	processes := []processSnapshot{
		...
		{PID: 15, PPID: 1, Command: "/old/cache/gimble-abc mcp"},
	}
	var killed []int
	retired, preserved, err := retireUnregisteredMCPProcesses(processes, map[int]bool{13: true}, func(pid int) error {
