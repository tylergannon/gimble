...
func (i pluginInstaller) install() error {
	...
	if legacyMarketplaceExists {
		if _, err := i.run("codex", "plugin", "marketplace", "remove", tractorMarketplaceName, "--json"); err != nil {
			return err
		}
	}
	...
	for _, legacyCache := range []string{
		...
		if err := os.RemoveAll(legacyCache); err != nil {
			return fmt.Errorf("remove legacy Gimble plugin cache %q: %w", legacyCache, err)
		}
	}
	i.output("installed %s; retired %d superseded and %d idle legacy MCP server(s); preserved detached runs and %d legacy run owner(s)\n",
		gimblePluginSelector, retired, legacyRetired, legacyPreserved)
...
func retireLegacyMCPProcesses(processes []processSnapshot, registered map[int]bool, kill func(int) error) (int, int, error) {
	...
	for _, process := range processes {
		if registered[process.PID] || !isGimbleOrTractorMCPCommand(process.Command) {
			continue
		}
...
func isGimbleOrTractorMCPCommand(command string) bool {
	fields := strings.Fields(command)
	if len(fields) != 2 || fields[1] != "mcp" {
		return false
	}
	executable := filepath.Base(fields[0])
	return executable == "gimble" || strings.HasPrefix(executable, "gimble-") || executable == "tractor" || strings.HasPrefix(executable, "tractor-")
}
