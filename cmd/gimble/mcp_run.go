...
const (
	...
	mcpRunStateEnv     = "GIMBLE_MCP_STATE_DIR"
)
...
func gimbleStateRoot() (string, error) {
	if xdgState := strings.TrimSpace(os.Getenv("XDG_STATE_HOME")); xdgState != "" {
		...
		return filepath.Join(absolute, "gimble"), nil
	}
	home, err := os.UserHomeDir()
	if err != nil {
		return "", fmt.Errorf("resolve home directory for Gimble state: %w", err)
	}
	return filepath.Join(home, ".local", "state", "gimble"), nil
}
...
func registerMCPInstance(store *mcpRunStore) (*mcpInstance, error) {
	...
	record := mcpInstanceRecord{
		PID: os.Getpid(), Version: gimbleMCPVersion, StartedAt: time.Now().UTC(),
...
func (s *mcpRunStore) refresh(record mcpRunRecord) (mcpRunRecord, error) {
	...
	return s.update(record.ID, func(current *mcpRunRecord) error {
		...
		if current.Status == "STOPPING" {
			...
			current.Failure = "detached Gimble runner exited without recording a result"
		}
		return nil
	})
}
...
func processOwnsRun(pid int, runID string) error {
	...
	if err != nil {
		return fmt.Errorf("inspect Gimble runner process %d: %w", pid, err)
	}
	command := string(output)
	if !strings.Contains(command, "mcp-runner") ||
		(!strings.Contains(command, "--run-id "+runID) && !strings.Contains(command, "--run-id="+runID)) {
		return fmt.Errorf("process %d no longer belongs to Gimble run %s", pid, runID)
	}
	return nil
}
