...
func TestAskResumesNumberedQuestionWithoutRenumbering(t *testing.T) {
	...
	t.Setenv("TRACTOR_INTERVIEW_DIR", dir)
	t.Setenv("TRACTOR_RUN_DIR", "")
...
func TestAskRequiresInterviewDirectory(t *testing.T) {
	t.Setenv("TRACTOR_INTERVIEW_DIR", "")
	_, _, err := executeCommand("ask", "question.md")
	if err == nil || !strings.Contains(err.Error(), "--into") || !strings.Contains(err.Error(), "TRACTOR_INTERVIEW_DIR") {
