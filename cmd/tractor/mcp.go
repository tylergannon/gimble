...
func newTractorMCPServer() (*server.MCPServer, *tractorMCPServer, error) {
	...
	mcpServer.AddTool(mcp.NewTool("get_pipeline_schema",
		mcp.WithDescription("Return the current pipeline JSON Schema generated by Tractor's graph type. Fetch only when authoring or changing a graph; the examples need no schema work."),
	...
	mcpServer.AddTool(mcp.NewTool("steer_run",
		mcp.WithDescription("Send one text instruction to the active steerable agent turn in a Tractor run; when none is active it returns accepted: false. Steer only when new authoritative information appears or the run is leaving scope — a complete goal up front beats frequent correction."),
	...
	mcpServer.AddTool(mcp.NewTool("stop_run",
		mcp.WithDescription("Request interruption of an MCP-started Tractor run. Repeating the call after the graceful window escalates to a forced stop."),
